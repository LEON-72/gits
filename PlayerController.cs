...
public class Stone : MonoBehaviour
{
    ...
    void Update()
    {
        // GameManagerが入力とRollDice()の呼び出しを管理します。
    }

    public void RollDice() // public に変更
    {
        if (dicePrefab == null)
        {
    ...
    IEnumerator ApplyNodeEffect(int currentPosIndex)
    {
        ...
        if (nodeEffect != null)
        {
            ...
            else if (nodeEffect.effectType == NodeEffect.EffectType.SkipTurn)
            {
                ...
                // DisplaySkipTurnMessage() は HideBubbleAfterDelay を呼ぶので、
                // ここでは直接 StartCoroutine(HideBubbleAfterDelay...) は呼ばない
            }
            ...
            else if (nodeEffect.effectType == NodeEffect.EffectType.MoveForward) // 前に進む
            {
                int targetIndex = nodeEffect.effectValue;
                if (targetIndex >= 0 && targetIndex < currentRoute.childNodeList.Count)
                {
                    Vector3 targetPosition = currentRoute.childNodeList[targetIndex].position;
                    bubbleText.text += "\n" + ("2マス進む");
